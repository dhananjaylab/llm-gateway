...
from app.core.schema import UnifiedChatRequest, UnifiedStreamChunk
...
class FallbackRouter:
    ...
    async def execute_non_streaming(
        ...
        *,
        ...
        tier_or_model: str,
    ) -> tuple[ProviderAdapter, str, dict]:
        """
        Walks `chain` in order. Returns (adapter, provider_model, raw_response)
        for whichever link succeeded. Raises:
        ...
        for link in chain:
            ...
            try:
                ...
                await self._record_outcome(
                    provider=adapter.provider_name, model=provider_model, ok=False, latency_ms=latency_ms
                )
                attempts.append(
                    FallbackAttempt(
                        link=link,
                ...
                if not exc.retryable:
                    # Document 03: "a bad request is a bad request on every
                    # provider" — bubble up as-is, no further links tried.
                    raise
                continue
            ...
            return adapter, provider_model, raw
